...
int num_of_staff = 2; //number of staffs
...
int insert_customer(int arrival_time)
{
	element customer;
...
int remove_customer()
{
	...
	customer = dequeue(&queue); //next customer
	service_time = customer.service_time - 1; //As sevice starts, needed service_time is (service_time of next customer)-1 at this moment.
	served_customers++;
...
void main()
{
	...
	//make the matrix of service time
	int* service_time_mat = (int*)malloc(sizeof(int) * num_of_staff);
	memset(service_time_mat, 0, sizeof(int) * num_of_staff); //initialize the matrix
	while (clock < duration) {
		...
		//repeat for two staffs
		//Start the service whenbank staff 1 or bank staff 2 are available
		for (int i = 0; i < num_of_staff; i++) {
			// Check if the customer who is receiving the service is finished.
			if (service_time_mat[i] > 0) {	// the customer is receiving service from [i]th server.
				service_time_mat[i]--;
			}
			// no customer is receiving service. 
			// So, take out a customer from the queue and start the service.
			else {
				service_time_mat[i] = remove_customer(); //set service_time_mat[i] to next customer's service_time-1
				//the next customer starts to receive the service.
			}
		}
	}
	print_stat();
