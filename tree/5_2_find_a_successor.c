...
TreeNode* tree_successor(TreeNode* p) {
	if (p->right != NULL) { //return the leftmost node of right subtree
		p = p->right; //move to right subtree
		while (p->left) p = p->left; //find the leftmost
		return p;
	}
	TreeNode *x = p->parent;
	while (x != NULL && p == x->right) { //break the loop when x is null or p !=x->right
		p = x;
		x = x->parent;
	}
	return x;
